...
void FffGcodeWriter::processSkirtBrim(const SliceDataStorage& storage, LayerPlan& gcode_layer, unsigned int extruder_nr) const
{
    ...
    const coord_t searching_radius = line_w * 3 / 2;
    ...
    {
        const SkirtBrimLine& line = storage.skirt_brim[extruder_nr][inset_idx];
        for (bool closed : { false, true })
        {
            for (ConstPolygonRef line : closed? line.closed_polygons : line.open_polylines)
            {
